	...
	static FINDLIST *xt11=NULL;
				 ...
				 for(it11=firstt11;it11!=NULL;it11=it11->next)
				 {
					 ...
					 if(gl_object_isa(it11,"battery"))// && it11!=OBJECTHDR(this))
				     {
						...
						if(*clear_price>= *new_price && *clear_quantity >= Energy/1000 && Energy>= E_Max*0.2 ) //checks if battery is in the market and takes the total power from the batteries that are in the market
						{
	...
	///////////////////////////////// SEARCH ALL SOLARS/////////////////////////////////
				 ...
				 for(it111=firstt111;it111!=NULL;it111=it111->next)
				 {
					 ...
					 if(gl_object_isa(it111,"solar") )
				     {
						...
						complex *solar=gl_get_complex_by_name(it111,"VA_Out"); //takes the total solar output
	...
	if(first_time==2)
	{
		if( battery_contr_object!=NULL)
		{
					 //system("pause");
						...
						if(*clear_price>= price && *clear_quantity >= Energy/4000 && Energy>= E_Max*0.2 )  //battery is in the market; it must be discharged
						{					
		...
		//that if else checks if battery is attached on solar or wind
		...
		real_wind=gl_get_double_by_name(wind_object,"TotalRealPow");  //this only one and no total like solar because my glm has only one wind
		
		
		power_absolute_total=(*real_wind+solar_total)/1000; //total output for both wind and solar
	      
			 clear_price=gl_get_double_by_name(auction_object,"current_market.clearing_price");
			 clear_quantity=gl_get_double_by_name(auction_object,"current_market.clearing_quantity");
		

			 if(*clear_price >=*price_of_charged && *clear_quantity >= power_absolute_total) //check about the solar and wind if they are in the market
			 {
			 ...
			 else if(*clear_price >= *price_of_charged && *clear_quantity <= power_absolute_total)
			{
					...
					if(wind_or_solar==1 && (*clear_quantity*1000 -total_power_form_batteries) < solar_total) //here i am attached on solar
					{
					   ...
					   if(battery_state==3) //tha basic battery is full then the stand by battery of solar which is partially charged, starts charging
						{
								if(fabs(B_scheduled_power2) >0)
								{
							

									//check absolute the solar panels
					...
					}//take from wind if the solar is not give the full energy
					else if (wind_or_solar==1 && (*clear_quantity*1000 -total_power_form_batteries) >= solar_total)  
					{
					...
					// i am attached on wind, and the battery of wind is charging
					if(wind_or_solar==0) //eimai wind kai se authn thn periptwsh tha fortizw thn mpataria tou wind
					{
						if(take_from_wind > 0) //i took energy from wind for demand satisfaction
						{
						...
						//tha basic battery is full then the stand by battery of wind which is partially charged, starts charging
						if(battery_state==3)
						{
								if(fabs(B_scheduled_power2) >0)
								{
			 ...
			 else if(*clear_price < *price_of_charged)  //solar-wind are not in market, so the battery charges
			 {
				// system("pause");
				 ...
				 if(*real_power <= 0)//in the case of solar
				 {
