...
auction::auction(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int32, "time_At_15min", PADDR(time_At_15min), PT_DESCRIPTION, "time_At_15min use from virtual_battery",
...
TIMESTAMP auction::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (t1>=clearat)
	{
		...
		time_At_15min=gl_tominutes(clearat);
		printf("in auction:%d %s %s",time_At_15min,timebuf,timebuf2);
